...
impl YojanaServer {
    ...
    #[tool(
        description = "Create, get, update, or comment on tasks. Actions: create (requires project, title), get (requires id — UUID or 'project-slug/N'), update (requires id, plus fields to change), comment (requires id, text; optional author). Fields outside an action's contract are rejected, not ignored; project is immutable after create. create/update return a slim ack {id, human_id, status, title}; use action=get (or yojana_context) for full detail. acceptance_criteria, decisions, context_refs are JSON arrays; acceptance_criteria items are {text, done} on read, and an item {_unreadable, raw} means the stored value failed to parse — that is a spec you cannot read, never an absent one. The string detail fields (category, slice_type, implementation_plan, execution_record, reproduction, root_cause) accept null to clear. arc_id + arc_phase must be provided together to assign a task to an arc phase. commit is a SHA shorthand that appends a git:commit context_ref (use with update, typically alongside status=done). Close-out capture (mined for cross-project lessons — write for a reader with no transcript): closing a bug REQUIRES root_cause stating the mechanism (why it broke, 1-3 sentences, not the fix — the fix goes in execution_record; if unknown, write 'unknown:' plus what was ruled out); set execution_record when execution diverged from plan (failed approaches, surprises, workarounds — omit when uneventful); status=wontfix requires a comment saying why (rejected approaches are negative knowledge); decisions entries carry rationale and the strongest rejected alternative."
    )]
    ...
    #[tool(
        description = "Find tasks ready to start — status is ready-for-agent or ready-for-human with all depends_on targets done. Optional: project (id or slug) to scope to one project and its sub-projects; omit for cross-project (all active projects)."
    )]
